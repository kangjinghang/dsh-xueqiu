...
- **登录雪球（可选）**：点面板右上「👤 登录」，按提示从浏览器复制 Cookie 整行粘贴即可。登录后自选股切换为你的**雪球云端自选股（云端为准）**；「同步云端自选」可随时手动拉取。注意：雪球已风控封禁第三方写入自选的接口（读接口正常），本地加/删自选会在下次同步时被云端列表覆盖——想改动自选请去雪球网页/App 操作。Cookie 只保存在本机，退出登录即清除。
