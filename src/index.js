  ...
  apply(ctx) {
    ...
    ctx.effect(function () {
      ...
      if (!connection || !connection.rpc) {
        console.error('[dsh-xueqiu] connection 服务不可用，Host RPC 未注册')
        return
      }
      return connection.rpc.handle('/xueqiu', async function (endpoint, payload, signal) {
        const reqObj = (payload && typeof payload === 'object') ? payload : {}
        const action = reqObj.action
        const args = reqObj.args || {}
        if (!dispatch[action]) {
          return { ok: false, error: { code: 'internal', message: '未知操作: ' + action, details: {} } }
        }
        try {
          const data = await dispatch[action](args)
          return { ok: true, value: sanitize(data) }
        } catch (e) {
